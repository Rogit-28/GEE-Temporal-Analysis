...
class ImageCache:
    ...
    def __init__(self, config: Config):
        ...
        logger.info(
            f"Initialized cache: {self.cache_dir} (max: {format_file_size(self.max_size_bytes)})"
        )
    ...
    def stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            cache_stats = {
                "hits": (
                    cache_stats_tuple[0]
                    if isinstance(cache_stats_tuple, tuple)
                    else cache_stats_tuple.get("hits", 0)
                ),
                "misses": (
                    cache_stats_tuple[1]
                    if isinstance(cache_stats_tuple, tuple)
                    else cache_stats_tuple.get("misses", 0)
                ),
            }
            ...
            return {
                ...
                "usage_percent": (
                    round((volume / self.max_size_bytes) * 100, 2)
    ...
    def cleanup(self) -> bool:
        ...
        try:
            ...
            removed_count = 0
            # diskcache.Cache does not expose dict-style keys(); iterate directly.
