...
class SatChangeWebAPI:
    ...
    def analyze_location(self, location_data: Dict) -> Dict:
        ...
        try:
            ...
            analysis_id = self._store_analysis_results(
                ...
                start_date, end_date, change_type, threshold,
                stats
            )
            
            # Generate visualization files
            viz_files = self._generate_visualization_files(
                analysis_id, processed_a, processed_b,
                change_summary['classification'], stats,
    ...
    def _store_analysis_results(self, location_name: str, center_lat: float, center_lon: float,
                               start_date: str, end_date: str, change_type: str, threshold: float,
                               stats: Dict) -> int:
    ...
    def _generate_visualization_files(self, analysis_id: int, bands_a: Dict, bands_b: Dict,
                                    classification: np.ndarray, stats: Dict,
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            viz_files['static_png'] = os.path.join(temp_dir, f'{analysis_id}_static.png')
            viz_files['interactive_html'] = os.path.join(temp_dir, f'{analysis_id}_interactive.html')
            viz_files['geotiff'] = os.path.join(temp_dir, f'{analysis_id}_classification.tif')
            viz_files['stats_json'] = os.path.join(temp_dir, f'{analysis_id}_stats.json')
    ...
    def get_analysis_history(self, location_name: Optional[str] = None, 
...
class SatChangeDataPipeline:
        ...
        for _, row in locations_df.iterrows():
            ...
            if result['success']:
                results.append({
                    ...
                    'lat': row['lat'],
                    'lon': row['lon'],
                    'total_change_percent': result['statistics']['total_change']['percent'],
                    'vegetation_loss_percent': result['statistics'].get('vegetation_loss', {}).get('percent', 0),
                    'vegetation_growth_percent': result['statistics'].get('vegetation_growth', {}).get('percent', 0),
                    'water_expansion_percent': result['statistics'].get('water_expansion', {}).get('percent', 0),
                    'water_reduction_percent': result['statistics'].get('water_reduction', {}).get('percent', 0),
    ...
    def generate_summary_report(self, results_df: pd.DataFrame) -> Dict:
        ...
        return {
            ...
            'top_locations_by_change': results_df.nlargest(5, 'total_change_percent')[['location', 'total_change_percent']].to_dict('records')
        }
            ...
            'features': [
                {
                    ...
                    'geometry': {
                        'type': 'Polygon',
                        'coordinates': [[[80.26, 13.08], [80.28, 13.08], [80.28, 13.10], [80.26, 13.10], [80.26, 13.08]]]
                    },
                    'properties': {
                ...
                },
                {
                    ...
                    'geometry': {
                        'type': 'Polygon',
                        'coordinates': [[[80.27, 13.09], [80.29, 13.09], [80.29, 13.11], [80.27, 13.11], [80.27, 13.09]]]
                    },
                    'properties': {
...
def create_sample_locations_dataframe() -> pd.DataFrame:
    ...
    return pd.DataFrame([
        {'name': 'chennai', 'lat': 13.0827, 'lon': 80.2707, 'start_date': '2020-01-01', 'end_date': '2024-12-31'},
        {'name': 'mumbai', 'lat': 19.0760, 'lon': 72.8777, 'start_date': '2020-01-01', 'end_date': '2024-12-31'},
        {'name': 'delhi', 'lat': 28.7041, 'lon': 77.1025, 'start_date': '2020-01-01', 'end_date': '2024-12-31'},
        {'name': 'bangalore', 'lat': 12.9716, 'lon': 77.5946, 'start_date': '2020-01-01', 'end_date': '2024-12-31'},
        {'name': 'kolkata', 'lat': 22.5726, 'lon': 88.3639, 'start_date': '2020-01-01', 'end_date': '2024-12-31'}
    ])
...
"""
    ...
    try:
        ...
        for file in ['analysis_results.csv', 'change_polygons.shp', 'workflow_results.json']:
