...
from satchange.gee_client import GEEClient, GEEError
from satchange.cache import CacheManager
from satchange.image_processor import ImageProcessor, PreprocessingError
...
class SatChangeAdvancedAnalyzer:
    ...
    def __init__(self, config_file: Optional[str] = None):
        ...
        self.visualization_manager = VisualizationManager(
            emboss_intensity=self.config.get('analysis_parameters.emboss_intensity', 1.0)
        )
    ...
    def analyze_location(self, location: Dict, analysis_params: Dict) -> Dict:
        ...
        try:
            ...
            print(f"Successfully analyzed {location['name']} in {result['metadata']['processing_time']:.2f}s")
    ...
    def compare_analysis_parameters(self, location: Dict, param_sets: List[Dict]) -> Dict:
        """
    ...
    def _generate_comparison_summary(self, results: List[Dict]) -> Dict:
        ...
        if successful_results:
            processing_times = [r['result']['metadata']['processing_time'] for r in successful_results]
            summary['best_performance'] = successful_results[np.argmin(processing_times)]['param_set']
            
            # Find most sensitive (highest change detection)
            total_changes = [r['result']['statistics']['total_change']['percent'] for r in successful_results]
            summary['most_sensitive'] = successful_results[np.argmax(total_changes)]['param_set']
            
            # Find most conservative (lowest change detection)
            summary['most_conservative'] = successful_results[np.argmin(total_changes)]['param_set']
...
"""
    ...
    try:
        ...
        comparison_results = analyzer.compare_analysis_parameters(comparison_location, param_sets)
        ...
        cached_results = analyzer.batch_analyze(locations[:2], batch_params)
